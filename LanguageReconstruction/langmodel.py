# Original code: https://github.com/ollie283/language-models
...
class TrigramLanguageModel(BigramLanguageModel):
    def __init__(self, sentences, smoothing=False):
        ...
        for sentence in sentences:
            ...
            previous_word2 = None
            for word in sentence:
                if previous_word1 != None and previous_word2 != None:
                    self.trigram_frequencies[(previous_word2, previous_word1, word)] = self.trigram_frequencies.get((previous_word2, previous_word1, word), 0)+ 1
                    if previous_word2 != SENTENCE_START and word != SENTENCE_END:
                        self.unique_trigrams.add((previous_word2, previous_word1, word))
    ...
    def calculate_trigram_probabilty(self, previous_word2, previous_word1, word):
        trigram_word_probability_numerator = self.trigram_frequencies.get((previous_word2, previous_word1, word), 0)
        trigram_word_probability_denominator = self.bigram_frequencies.get((previous_word2, previous_word1), 0)
        if self.smoothing:
    ...
    def calculate_trigram_sentence_probability(self, sentence, normalize_probability=True):
        ...
        for word in sentence:
            if previous_word != None:
                trigram_word_probability = self.calculate_trigram_probabilty(previous_word, word)
...
if __name__ == '__main__':
    toy_dataset = read_sentences_from_file("./train.txt")
    toy_dataset_test = read_sentences_from_file("./test.txt")
